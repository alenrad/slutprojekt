...
def powerup_remove(glo):
    glo["powerup_x"] = -1000
    glo["powerup_y"] = -1000
    glo["counter"] = 0
    
def powerup_reset(glo):
    glo["powerup_x"] = 400
    glo["powerup_y"] = 200
    glo["counter"] = 0
    glo["powerup_count"] = 0
    
def little_ball_reset(glo):
    glo["little_ball_x"] = 390
    glo["little_ball_y"] = 290
    
def bounce_back_ball_left(glo):
...
def bounce_back_ball_right(glo):
    glo["ball_dx"] = glo["ball_dx"] * -1
    angle_velocity = atan(glo["ball_dy"]/ glo["ball_dx"])
    angle_velocity += random(-1, 1) 
    velosity = (glo["ball_dy"]**2 + glo["ball_dx"]**2)**0.5
...
def check_and_perform_bounce_on_paddles(glo):
    ...
    if perform_bounce_left:
        bounce_back_ball_left(glo)
        glo["counter"] += 1
    if perform_bounce_right:
        bounce_back_ball_right(glo)
        glo["counter"] += 1
...
def check_and_perform_collision_with_balls(glo):
...
def check_and_perform_scoring(glo):
...
def check_balls_bounce_off_top_or_bottom(glo):
...
def check_if_paddle_touch_top_bottom(glo):
